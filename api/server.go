...
func NewServer (Store *db.Store) *server {
	...
	router.GET("/transactions/user/:userID", server.listUserTransactions)
	...
	router.GET("/fees/market/:marketID", server.getFeeByMarketID)
	...
	router.GET("/wallets/user/:userID", server.listWallets)
	router.PUT("/wallets/:id", server.updateWallet)
	router.DELETE("/wallets/:id", server.deleteWallet)

    router.GET("/audit-logs", server.listAuditLogs)
	router.GET("/audit-logs/user/:userID", server.userAuditLogs)
